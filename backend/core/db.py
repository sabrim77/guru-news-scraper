...
def _create_base_schema(conn: sqlite3.Connection) -> None:
    ...
    Table: news
        ...
        - pub_date         TEXT                    (RSS pub datetime as ISO string)
        - author           TEXT                    (parsed from article HTML, if any)
        - article_pub_date TEXT                    (HTML-level publication date, ISO)
        - summary          TEXT                    (short summary/standfirst, if any)
...
def insert_news(
    ...
    Args:
        ...
        pub_date:         Publication datetime from RSS as ISO string (optional).
        author:           Author/byline parsed from HTML (optional).
        article_pub_date: Publication datetime parsed from HTML (optional).
        summary:          Short summary / standfirst (optional).
...
def insert_articles(articles: List[Dict[str, Any]]) -> int:
    ...
    Expected keys in each article dict (not all required):
        ...
        Core fields:
            ...
            - "summary":       str | None   (short summary/standfirst)
        ...
        Source / metadata:
            ...
            - "author":        str | None   → stored as `author`

        Publication time:
            - "published_at":  datetime | str | None  (RSS or article)
            - "pub_date":      datetime | str | None
            - "article_pub_date": datetime | str | None

    Mapping to DB columns:
        - portal           ← article["source"] or article["portal"] or "unknown"
        - url              ← article["url"]
        - title            ← article["title"]
        - content          ← article["content"] (or summary if content missing)
        - topic            ← article["keyword"] or article["topic"]
        - pub_date         ← ISO string derived from `pub_date` / `published_at`
        - author           ← article["author"]
        - article_pub_date ← ISO from article["article_pub_date"] (if any)
    ...
    for a in articles:
        ...
        # Ensure content is not completely empty: if only summary exists,
        # we also copy it into content for search friendliness.
        if raw_content:
            content = raw_content
        elif summary:
            content = summary
        ...
        # Separate HTML-level article publication date if provided
        article_pub_date = a.get("article_pub_date")
        if article_pub_date is not None and not isinstance(article_pub_date, str):
            try:
                article_pub_date = article_pub_date.isoformat()  # type: ignore[attr-defined]
            except Exception:
                article_pub_date = str(article_pub_date)
...
def get_latest(
    ...
    """
    Return latest rows (ordered by newest first).

    Used by /api/v1/news/latest.

    Args:
        limit:  max rows
        offset: pagination offset
        portal: optional portal filter (e.g. "prothomalo", "bbc")
    """
    conn = _get_conn()
    cur = conn.cursor()
    ...
    if portal:
        cur.execute(
            ...
            WHERE portal = ?
            ORDER BY
                COALESCE(pub_date, article_pub_date, created_at) DESC,
                id DESC
            LIMIT ? OFFSET ?;
            """,
            (portal, limit, offset),
...
def get_latest_by_topic(
    ...
    """
    Return the latest rows for a given topic (keyword) stored in `news.topic`.

    Used by /api/v1/news/by_topic.
    """
