...
This module is focused on "search keywords" (what the user typed),
NOT on classifier topics ("politics", "sports", etc.). Classifier
topics are handled separately in the scraping pipeline (`core.runner`).
"""
...
def _parse_published(entry) -> Optional[datetime]:
    ...
    Returns:
        datetime instance or None if parsing fails/unavailable.
    """
...
def _entry_matches_keyword(entry: Any, keyword: str) -> bool:
    """
...
def _normalize_lang_for_portals(lang: Optional[str]) -> Optional[str]:
    """
...
def _normalize_country_for_portals(country: Optional[str]) -> Optional[str]:
    """
...
def fetch_news_for_keyword(
        ...
        Each article dict has at least:
            {
                ...
                "content": None,
                "source": portal_id,
                "keyword": keyword,        # stored for search/filter purposes
    ...
    # For this keyword, avoid duplicate (portal, url) inserts
    seen_links_for_kw = set()
    ...
    for portal_id, cfg in iter_enabled_portals():
        ...
        for rss_url in rss_list:
            ...
            for entry in getattr(feed, "entries", []):
                ...
                # Avoid duplicates for the same keyword from same portal
                key = (portal_id, link)
                if key in seen_links_for_kw:
                    continue
                ...
                articles.append(
                    {
                        ...
                        "keyword": keyword,       # stored as search keyword/tag in DB
                        "published_at": published_at,
                    }
                )
                seen_links_for_kw.add(key)
    ...
    Returns:
        {
          ...
          "total_fetched": 25,           # total articles across all keywords
          "by_keyword": {
            "bitcoin": [...],
            "tesla":   [...],
          }
